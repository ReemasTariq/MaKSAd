...
public class ParticipationView extends JFrame {
    ...
    private Timestamp mergeDateWithTime(String eventDate, LocalTime time) {
        LocalDate d = LocalDate.parse(eventDate);
        return Timestamp.valueOf(d.atTime(time));
    }
            ...
            while (rs.next()) {
                ...
                if (dbStatus != null) {
                    ...
                    if (dbStatus.equals("PRESENT") ||
                        dbStatus.equals("ABSENT")  ||
                        dbStatus.equals("CANCELED")||
                        dbStatus.equals("UNSET")) {
                        uiStatus = dbStatus;
                    } else {
                     
                        if (hoursVal != null && hoursVal > 0) uiStatus = "PRESENT";
                        else uiStatus = "UNSET";
                    }
                } else {
    ...
    private void saveChangesToDatabase() {
        ...
        try (Connection conn      = DBConnection.getConnection();
             ...
             PreparedStatement psCertChk = conn.prepareStatement(certCheckSql);
             PreparedStatement psCertIns = conn.prepareStatement(certInsertSql)) {
            ...
            for (int i = 0; i < model.getRowCount(); i++) {
                ...
                String status      = statusObj   == null ? "" : statusObj.toString().trim().toUpperCase();
                ...
                if ("PRESENT".equalsIgnoreCase(status)) {
                    ...
                    if (ci == null || co == null) {
                        ...
                        try (ResultSet ers = evPs.executeQuery()) {
                            if (ers.next()) {
                                Timestamp dbStart = ers.getTimestamp("start_time");
                                Timestamp dbEnd   = ers.getTimestamp("end_time");

                                if (ci == null && dbStart != null) {
                                    LocalTime startTime = dbStart.toLocalDateTime().toLocalTime();
                                    ci = mergeDateWithTime(eventDate, startTime);
                                }
                                if (co == null && dbEnd != null) {
                                    LocalTime endTime = dbEnd.toLocalDateTime().toLocalTime();
                                    co = mergeDateWithTime(eventDate, endTime);
                                }
                            }
                        }
                    }
                ...
                psUpdate.setTimestamp(3, ci);
                psUpdate.setTimestamp(4, co);

                if (hours != null) psUpdate.setDouble(5, hours);
                else psUpdate.setNull(5, Types.DOUBLE);

                psUpdate.setInt(6, volunteerId);
                psUpdate.setString(7, eventName);
                psUpdate.setString(8, eventDate);

                psUpdate.addBatch();

              
                if ("PRESENT".equalsIgnoreCase(status) && hours != null) {
                    java.sql.Date issueDateSql = java.sql.Date.valueOf(eventDate);

                    psCertChk.setInt(1, volunteerId);
                    psCertChk.setString(2, eventName);
                    psCertChk.setDate(3, issueDateSql);

                    try (ResultSet rsCert = psCertChk.executeQuery()) {
                        if (!rsCert.next()) {
                            psCertIns.setInt(1, volunteerId);
                            psCertIns.setString(2, eventName);
                            psCertIns.setDate(3, issueDateSql);
                            psCertIns.setDouble(4, hours);
                            psCertIns.executeUpdate();
                        }
                    }
                }
            }
